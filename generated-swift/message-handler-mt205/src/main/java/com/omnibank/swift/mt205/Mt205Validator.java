...
public final class Mt205Validator {
    ...
    public ValidationResult validate(Mt205Message message) {
    ...
    private void checkPaymentInvariants(Mt205Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt205Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT205",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt205Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
