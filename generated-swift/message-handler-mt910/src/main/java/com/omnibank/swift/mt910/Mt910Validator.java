...
public final class Mt910Validator {
    ...
    public ValidationResult validate(Mt910Message message) {
    ...
    private void checkPaymentInvariants(Mt910Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt910Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT910",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt910Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
