...
public final class Mt942Validator {
    ...
    public ValidationResult validate(Mt942Message message) {
    ...
    private void checkPaymentInvariants(Mt942Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt942Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT942",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt942Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
