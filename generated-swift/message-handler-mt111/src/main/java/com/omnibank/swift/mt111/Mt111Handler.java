...
/**
 * End-to-end handler for SWIFT MT111. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt111Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt111RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt111Parser parser = new Mt111Parser();
    private final Mt111Validator validator = new Mt111Validator();
    private final Mt111RoutingRules routing = new Mt111RoutingRules();
    ...
    private String summarizeErrors(Mt111Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt111Validator.ValidationFinding.Severity.ERROR) {
