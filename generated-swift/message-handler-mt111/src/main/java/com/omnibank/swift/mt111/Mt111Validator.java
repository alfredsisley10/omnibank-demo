...
public final class Mt111Validator {
    ...
    public ValidationResult validate(Mt111Message message) {
    ...
    private void checkPaymentInvariants(Mt111Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt111Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT111",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt111Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
