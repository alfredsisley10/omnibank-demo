...
public final class Mt202COVValidator {
    ...
    public ValidationResult validate(Mt202COVMessage message) {
    ...
    private void checkPaymentInvariants(Mt202COVMessage m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt202COVMessage m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT202COV",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt202COVMessage m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
