...
public final class Mt112Validator {
    ...
    public ValidationResult validate(Mt112Message message) {
    ...
    private void checkPaymentInvariants(Mt112Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt112Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT112",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt112Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
