...
public final class Mt754Validator {
    ...
    public ValidationResult validate(Mt754Message message) {
    ...
    private void checkPaymentInvariants(Mt754Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt754Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT754",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt754Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
