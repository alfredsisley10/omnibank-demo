...
public final class Mt103Validator {
    ...
    public ValidationResult validate(Mt103Message message) {
    ...
    private void checkPaymentInvariants(Mt103Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt103Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT103",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt103Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
