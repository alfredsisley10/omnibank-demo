...
/**
 * End-to-end handler for SWIFT MT103. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt103Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt103RoutingRules.Destination destination,
    ...
    private final Mt103RoutingRules routing = new Mt103RoutingRules();
    ...
    private String summarizeErrors(Mt103Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt103Validator.ValidationFinding.Severity.ERROR) {
