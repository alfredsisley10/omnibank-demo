...
public final class Mt340Validator {
    ...
    public ValidationResult validate(Mt340Message message) {
    ...
    private void checkPaymentInvariants(Mt340Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt340Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT340",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt340Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
