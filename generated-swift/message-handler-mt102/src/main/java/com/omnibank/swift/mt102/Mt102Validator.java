...
public final class Mt102Validator {
    ...
    public ValidationResult validate(Mt102Message message) {
    ...
    private void checkPaymentInvariants(Mt102Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt102Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT102",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt102Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
