...
/**
 * End-to-end handler for SWIFT MT102. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt102Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt102RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt102Parser parser = new Mt102Parser();
    private final Mt102Validator validator = new Mt102Validator();
    private final Mt102RoutingRules routing = new Mt102RoutingRules();
    ...
    private String summarizeErrors(Mt102Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt102Validator.ValidationFinding.Severity.ERROR) {
