...
/**
 * End-to-end handler for SWIFT MT450. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt450Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt450RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt450Parser parser = new Mt450Parser();
    private final Mt450Validator validator = new Mt450Validator();
    private final Mt450RoutingRules routing = new Mt450RoutingRules();
    ...
    private String summarizeErrors(Mt450Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt450Validator.ValidationFinding.Severity.ERROR) {
