...
public final class Mt450Validator {
    ...
    public ValidationResult validate(Mt450Message message) {
    ...
    private void checkPaymentInvariants(Mt450Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt450Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT450",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt450Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
