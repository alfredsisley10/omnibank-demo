...
public final class Mt101Validator {
    ...
    public ValidationResult validate(Mt101Message message) {
    ...
    private void checkPaymentInvariants(Mt101Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt101Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT101",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt101Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
