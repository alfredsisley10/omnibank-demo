...
/**
 * End-to-end handler for SWIFT MT101. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt101Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt101RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt101Parser parser = new Mt101Parser();
    private final Mt101Validator validator = new Mt101Validator();
    private final Mt101RoutingRules routing = new Mt101RoutingRules();
    ...
    private String summarizeErrors(Mt101Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt101Validator.ValidationFinding.Severity.ERROR) {
