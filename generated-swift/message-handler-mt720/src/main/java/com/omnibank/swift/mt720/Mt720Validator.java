...
public final class Mt720Validator {
    ...
    public ValidationResult validate(Mt720Message message) {
    ...
    private void checkPaymentInvariants(Mt720Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt720Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT720",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt720Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
