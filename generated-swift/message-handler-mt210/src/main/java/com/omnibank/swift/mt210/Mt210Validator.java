...
public final class Mt210Validator {
    ...
    public ValidationResult validate(Mt210Message message) {
    ...
    private void checkPaymentInvariants(Mt210Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt210Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT210",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt210Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
