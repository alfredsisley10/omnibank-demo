...
public final class Mt290Validator {
    ...
    public ValidationResult validate(Mt290Message message) {
    ...
    private void checkPaymentInvariants(Mt290Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt290Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT290",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt290Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
