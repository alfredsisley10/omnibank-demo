...
public final class Mt107Validator {
    ...
    public ValidationResult validate(Mt107Message message) {
    ...
    private void checkPaymentInvariants(Mt107Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt107Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT107",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt107Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
