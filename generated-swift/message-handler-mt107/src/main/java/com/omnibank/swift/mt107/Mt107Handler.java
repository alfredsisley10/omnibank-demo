...
/**
 * End-to-end handler for SWIFT MT107. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt107Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt107RoutingRules.Destination destination,
    ...
    private String summarizeErrors(Mt107Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt107Validator.ValidationFinding.Severity.ERROR) {
