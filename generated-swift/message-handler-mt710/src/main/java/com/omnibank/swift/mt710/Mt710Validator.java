...
public final class Mt710Validator {
    ...
    public ValidationResult validate(Mt710Message message) {
    ...
    private void checkPaymentInvariants(Mt710Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt710Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT710",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt710Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
