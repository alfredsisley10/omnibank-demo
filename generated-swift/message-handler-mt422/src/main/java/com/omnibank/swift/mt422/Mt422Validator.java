...
public final class Mt422Validator {
    ...
    public ValidationResult validate(Mt422Message message) {
    ...
    private void checkPaymentInvariants(Mt422Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt422Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT422",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt422Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
