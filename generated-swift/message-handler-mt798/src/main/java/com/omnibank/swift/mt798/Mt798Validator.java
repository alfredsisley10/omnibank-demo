...
public final class Mt798Validator {
    ...
    public ValidationResult validate(Mt798Message message) {
    ...
    private void checkPaymentInvariants(Mt798Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt798Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT798",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt798Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
