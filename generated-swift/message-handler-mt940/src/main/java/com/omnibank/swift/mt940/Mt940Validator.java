...
public final class Mt940Validator {
    ...
    public ValidationResult validate(Mt940Message message) {
    ...
    private void checkPaymentInvariants(Mt940Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt940Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT940",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt940Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
