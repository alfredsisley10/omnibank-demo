...
public final class Mt330Validator {
    ...
    public ValidationResult validate(Mt330Message message) {
    ...
    private void checkPaymentInvariants(Mt330Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt330Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT330",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt330Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
