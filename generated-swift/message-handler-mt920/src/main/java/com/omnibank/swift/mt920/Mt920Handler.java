...
/**
 * End-to-end handler for SWIFT MT920. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt920Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt920RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt920Parser parser = new Mt920Parser();
    private final Mt920Validator validator = new Mt920Validator();
    private final Mt920RoutingRules routing = new Mt920RoutingRules();
    ...
    private String summarizeErrors(Mt920Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt920Validator.ValidationFinding.Severity.ERROR) {
