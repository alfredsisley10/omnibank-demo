...
public final class Mt920Validator {
    ...
    public ValidationResult validate(Mt920Message message) {
    ...
    private void checkPaymentInvariants(Mt920Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt920Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT920",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt920Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
