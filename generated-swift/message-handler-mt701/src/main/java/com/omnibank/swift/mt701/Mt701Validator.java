...
public final class Mt701Validator {
    ...
    public ValidationResult validate(Mt701Message message) {
    ...
    private void checkPaymentInvariants(Mt701Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt701Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT701",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt701Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
