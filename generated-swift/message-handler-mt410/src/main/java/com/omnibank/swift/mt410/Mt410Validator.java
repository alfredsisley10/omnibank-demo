...
public final class Mt410Validator {
    ...
    public ValidationResult validate(Mt410Message message) {
    ...
    private void checkPaymentInvariants(Mt410Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt410Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT410",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt410Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
