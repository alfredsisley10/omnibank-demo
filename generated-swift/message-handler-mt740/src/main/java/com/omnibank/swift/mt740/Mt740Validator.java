...
public final class Mt740Validator {
    ...
    public ValidationResult validate(Mt740Message message) {
    ...
    private void checkPaymentInvariants(Mt740Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt740Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT740",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt740Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
