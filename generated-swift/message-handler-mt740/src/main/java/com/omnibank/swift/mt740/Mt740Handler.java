...
/**
 * End-to-end handler for SWIFT MT740. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt740Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt740RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt740Parser parser = new Mt740Parser();
    private final Mt740Validator validator = new Mt740Validator();
    private final Mt740RoutingRules routing = new Mt740RoutingRules();
    ...
    private String summarizeErrors(Mt740Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt740Validator.ValidationFinding.Severity.ERROR) {
