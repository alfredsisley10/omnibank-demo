...
public final class Mt750Validator {
    ...
    public ValidationResult validate(Mt750Message message) {
    ...
    private void checkPaymentInvariants(Mt750Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt750Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT750",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt750Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
