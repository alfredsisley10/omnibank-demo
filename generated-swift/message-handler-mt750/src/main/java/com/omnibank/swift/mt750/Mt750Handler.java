...
/**
 * End-to-end handler for SWIFT MT750. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt750Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt750RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt750Parser parser = new Mt750Parser();
    private final Mt750Validator validator = new Mt750Validator();
    private final Mt750RoutingRules routing = new Mt750RoutingRules();
    ...
    private String summarizeErrors(Mt750Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt750Validator.ValidationFinding.Severity.ERROR) {
