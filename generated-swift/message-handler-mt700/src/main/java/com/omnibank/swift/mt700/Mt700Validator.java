...
public final class Mt700Validator {
    ...
    public ValidationResult validate(Mt700Message message) {
    ...
    private void checkPaymentInvariants(Mt700Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt700Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT700",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt700Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
