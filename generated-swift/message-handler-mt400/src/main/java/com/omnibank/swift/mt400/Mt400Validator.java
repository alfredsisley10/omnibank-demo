...
public final class Mt400Validator {
    ...
    public ValidationResult validate(Mt400Message message) {
    ...
    private void checkPaymentInvariants(Mt400Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt400Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT400",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt400Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
