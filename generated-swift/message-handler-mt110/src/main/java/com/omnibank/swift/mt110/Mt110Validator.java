...
public final class Mt110Validator {
    ...
    public ValidationResult validate(Mt110Message message) {
    ...
    private void checkPaymentInvariants(Mt110Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt110Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT110",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt110Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
