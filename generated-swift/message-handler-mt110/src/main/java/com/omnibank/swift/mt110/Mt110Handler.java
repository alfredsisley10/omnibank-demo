...
/**
 * End-to-end handler for SWIFT MT110. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt110Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt110RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt110Parser parser = new Mt110Parser();
    private final Mt110Validator validator = new Mt110Validator();
    private final Mt110RoutingRules routing = new Mt110RoutingRules();
    ...
    private String summarizeErrors(Mt110Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt110Validator.ValidationFinding.Severity.ERROR) {
