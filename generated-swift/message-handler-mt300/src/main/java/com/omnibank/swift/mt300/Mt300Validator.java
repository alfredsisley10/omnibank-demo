...
public final class Mt300Validator {
    ...
    public ValidationResult validate(Mt300Message message) {
    ...
    private void checkPaymentInvariants(Mt300Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt300Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT300",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt300Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
