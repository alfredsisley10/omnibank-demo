...
/**
 * End-to-end handler for SWIFT MT300. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt300Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt300RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt300Parser parser = new Mt300Parser();
    private final Mt300Validator validator = new Mt300Validator();
    private final Mt300RoutingRules routing = new Mt300RoutingRules();
    ...
    private String summarizeErrors(Mt300Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt300Validator.ValidationFinding.Severity.ERROR) {
