...
public final class Mt420Validator {
    ...
    public ValidationResult validate(Mt420Message message) {
    ...
    private void checkPaymentInvariants(Mt420Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt420Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT420",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt420Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
