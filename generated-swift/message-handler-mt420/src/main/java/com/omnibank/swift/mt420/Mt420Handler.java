...
/**
 * End-to-end handler for SWIFT MT420. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt420Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt420RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt420Parser parser = new Mt420Parser();
    private final Mt420Validator validator = new Mt420Validator();
    private final Mt420RoutingRules routing = new Mt420RoutingRules();
    ...
    private String summarizeErrors(Mt420Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt420Validator.ValidationFinding.Severity.ERROR) {
