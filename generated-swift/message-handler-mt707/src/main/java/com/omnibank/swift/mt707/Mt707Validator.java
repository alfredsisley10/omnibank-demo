...
public final class Mt707Validator {
    ...
    public ValidationResult validate(Mt707Message message) {
    ...
    private void checkPaymentInvariants(Mt707Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt707Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT707",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt707Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
