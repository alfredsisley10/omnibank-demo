...
public final class Mt203Validator {
    ...
    public ValidationResult validate(Mt203Message message) {
    ...
    private void checkPaymentInvariants(Mt203Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt203Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT203",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt203Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
