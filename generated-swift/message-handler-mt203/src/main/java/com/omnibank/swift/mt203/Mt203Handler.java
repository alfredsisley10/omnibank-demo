...
/**
 * End-to-end handler for SWIFT MT203. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt203Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt203RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt203Parser parser = new Mt203Parser();
    private final Mt203Validator validator = new Mt203Validator();
    private final Mt203RoutingRules routing = new Mt203RoutingRules();
    ...
    private String summarizeErrors(Mt203Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt203Validator.ValidationFinding.Severity.ERROR) {
