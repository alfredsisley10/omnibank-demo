...
public final class Mt304Validator {
    ...
    public ValidationResult validate(Mt304Message message) {
    ...
    private void checkPaymentInvariants(Mt304Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt304Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT304",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt304Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
