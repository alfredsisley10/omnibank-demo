...
public final class Mt320Validator {
    ...
    public ValidationResult validate(Mt320Message message) {
    ...
    private void checkPaymentInvariants(Mt320Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt320Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT320",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt320Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
