...
public final class Mt760Validator {
    ...
    public ValidationResult validate(Mt760Message message) {
    ...
    private void checkPaymentInvariants(Mt760Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt760Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT760",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt760Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
