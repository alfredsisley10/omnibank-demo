...
public final class Mt900Validator {
    ...
    public ValidationResult validate(Mt900Message message) {
    ...
    private void checkPaymentInvariants(Mt900Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt900Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT900",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt900Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
