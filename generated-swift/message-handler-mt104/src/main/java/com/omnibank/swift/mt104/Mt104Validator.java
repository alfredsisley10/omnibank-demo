...
public final class Mt104Validator {
    ...
    public ValidationResult validate(Mt104Message message) {
    ...
    private void checkPaymentInvariants(Mt104Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt104Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT104",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt104Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
