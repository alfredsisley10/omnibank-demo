...
public final class Mt360Validator {
    ...
    public ValidationResult validate(Mt360Message message) {
    ...
    private void checkPaymentInvariants(Mt360Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt360Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT360",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt360Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
