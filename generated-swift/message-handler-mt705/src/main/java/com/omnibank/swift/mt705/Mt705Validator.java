...
public final class Mt705Validator {
    ...
    public ValidationResult validate(Mt705Message message) {
    ...
    private void checkPaymentInvariants(Mt705Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt705Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT705",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt705Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
