...
public final class Mt200Validator {
    ...
    public ValidationResult validate(Mt200Message message) {
    ...
    private void checkPaymentInvariants(Mt200Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt200Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT200",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt200Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
