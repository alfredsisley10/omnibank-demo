...
public final class Mt412Validator {
    ...
    public ValidationResult validate(Mt412Message message) {
    ...
    private void checkPaymentInvariants(Mt412Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt412Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT412",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt412Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
