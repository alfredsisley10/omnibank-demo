...
public final class Mt941Validator {
    ...
    public ValidationResult validate(Mt941Message message) {
    ...
    private void checkPaymentInvariants(Mt941Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt941Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT941",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt941Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
