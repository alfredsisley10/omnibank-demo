...
/**
 * End-to-end handler for SWIFT MT941. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt941Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt941RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt941Parser parser = new Mt941Parser();
    private final Mt941Validator validator = new Mt941Validator();
    private final Mt941RoutingRules routing = new Mt941RoutingRules();
    ...
    private String summarizeErrors(Mt941Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt941Validator.ValidationFinding.Severity.ERROR) {
