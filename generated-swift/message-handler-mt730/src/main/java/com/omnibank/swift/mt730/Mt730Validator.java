...
public final class Mt730Validator {
    ...
    public ValidationResult validate(Mt730Message message) {
    ...
    private void checkPaymentInvariants(Mt730Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt730Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT730",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt730Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
