...
/**
 * End-to-end handler for SWIFT MT730. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt730Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt730RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt730Parser parser = new Mt730Parser();
    private final Mt730Validator validator = new Mt730Validator();
    private final Mt730RoutingRules routing = new Mt730RoutingRules();
    ...
    private String summarizeErrors(Mt730Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt730Validator.ValidationFinding.Severity.ERROR) {
