...
public final class Mt202Validator {
    ...
    public ValidationResult validate(Mt202Message message) {
    ...
    private void checkPaymentInvariants(Mt202Message m, List<ValidationFinding> findings) {
    ...
    private void checkBeneficiaryPresent(Mt202Message m, List<ValidationFinding> findings) {
        if (m.beneficiary() == null || m.beneficiary().isBlank()) {
            findings.add(new ValidationFinding(
                    "MISSING_BENEFICIARY",
                    "Beneficiary (tag 59) is required for MT202",
                    ValidationFinding.Severity.ERROR));
        }
    }

    private void checkSettlementViable(Mt202Message m, List<ValidationFinding> findings) {
        if (m.valueDate() == null) {
            findings.add(new ValidationFinding(
