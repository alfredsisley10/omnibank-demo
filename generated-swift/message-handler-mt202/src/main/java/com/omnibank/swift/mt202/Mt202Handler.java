...
/**
 * End-to-end handler for SWIFT MT202. Coordinates parse →
 * validate → route → forward pipeline and records basic
 * telemetry.
 */
public final class Mt202Handler {

    public record HandleResult(
            boolean accepted,
            String messageReference,
            Mt202RoutingRules.Destination destination,
            String rejectionReason,
            Instant processedAt
    ) {}

    private final Mt202Parser parser = new Mt202Parser();
    private final Mt202Validator validator = new Mt202Validator();
    private final Mt202RoutingRules routing = new Mt202RoutingRules();
    ...
    private String summarizeErrors(Mt202Validator.ValidationResult vr) {
        StringBuilder sb = new StringBuilder();
        for (var f : vr.findings()) {
            if (f.severity() == Mt202Validator.ValidationFinding.Severity.ERROR) {
