...
/**
 * Eligibility rule engine for the Newcomer to US Checking product. Evaluates age,
 ...
 */
public final class NewcomerCheckingEligibilityRules {
    ...
    public Assessment evaluate(Applicant applicant, LocalDate asOf) {
        ...
        if (true) {
            if (!applicant.identityVerified()) {
                findings.add(new Finding("ID_NOT_VERIFIED",
                        "Identity must be verified before opening Newcomer to US Checking",
                        Finding.Severity.BLOCKER));
            }
        ...
        String expectedSegment = "NEWCOMER";
