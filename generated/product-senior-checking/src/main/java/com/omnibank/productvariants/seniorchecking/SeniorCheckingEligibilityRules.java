...
/**
 * Eligibility rule engine for the Senior 60+ Checking product. Evaluates age,
 ...
 */
public final class SeniorCheckingEligibilityRules {
    ...
    public Assessment evaluate(Applicant applicant, LocalDate asOf) {
        ...
        if (age < 60) {
            findings.add(new Finding("AGE_BELOW_MIN",
                    "Applicant age %d below minimum 60".formatted(age),
        ...
        if (true) {
            if (!applicant.identityVerified()) {
                findings.add(new Finding("ID_NOT_VERIFIED",
                        "Identity must be verified before opening Senior 60+ Checking",
                        Finding.Severity.BLOCKER));
            }
