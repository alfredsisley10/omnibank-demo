...
/**
 * Eligibility rule engine for the Teen Checking product. Evaluates age,
 ...
 */
public final class TeenCheckingEligibilityRules {
    ...
    public Assessment evaluate(Applicant applicant, LocalDate asOf) {
        ...
        if (age < 13) {
            findings.add(new Finding("AGE_BELOW_MIN",
                    "Applicant age %d below minimum 13".formatted(age),
        ...
        if (true) {
            if (!applicant.identityVerified()) {
                findings.add(new Finding("ID_NOT_VERIFIED",
                        "Identity must be verified before opening Teen Checking",
                        Finding.Severity.BLOCKER));
            }
