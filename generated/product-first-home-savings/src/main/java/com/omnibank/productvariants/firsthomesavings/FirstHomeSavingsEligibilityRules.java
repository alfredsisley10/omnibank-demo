...
/**
 * Eligibility rule engine for the First Home Savings Program product. Evaluates age,
 ...
 */
public final class FirstHomeSavingsEligibilityRules {
    ...
    public Assessment evaluate(Applicant applicant, LocalDate asOf) {
        ...
        if (age > 40) {
            findings.add(new Finding("AGE_ABOVE_MAX",
                    "Applicant age %d above maximum 40".formatted(age),
                    Finding.Severity.BLOCKER));
        }
        ...
        if (true) {
            if (!applicant.identityVerified()) {
                findings.add(new Finding("ID_NOT_VERIFIED",
                        "Identity must be verified before opening First Home Savings Program",
                        Finding.Severity.BLOCKER));
            }
