...
/**
 * Eligibility rule engine for the Coverdell Education Savings product. Evaluates age,
 ...
 */
public final class CoverdellEsaEligibilityRules {
    ...
    public Assessment evaluate(Applicant applicant, LocalDate asOf) {
        ...
        if (age < 0) {
            findings.add(new Finding("AGE_BELOW_MIN",
                    "Applicant age %d below minimum 0".formatted(age),
                    Finding.Severity.BLOCKER));
        }
        if (age > 17) {
            findings.add(new Finding("AGE_ABOVE_MAX",
                    "Applicant age %d above maximum 17".formatted(age),
                    Finding.Severity.BLOCKER));
        }
        ...
        if (true) {
            if (!applicant.identityVerified()) {
                findings.add(new Finding("ID_NOT_VERIFIED",
                        "Identity must be verified before opening Coverdell Education Savings",
                        Finding.Severity.BLOCKER));
            }
