...
/**
 * Eligibility rule engine for the Escrow Money Market product. Evaluates age,
 ...
 */
public final class EscrowMoneyMarketEligibilityRules {
        ...
        if (true) {
            if (!applicant.identityVerified()) {
                findings.add(new Finding("ID_NOT_VERIFIED",
                        "Identity must be verified before opening Escrow Money Market",
                        Finding.Severity.BLOCKER));
            }
