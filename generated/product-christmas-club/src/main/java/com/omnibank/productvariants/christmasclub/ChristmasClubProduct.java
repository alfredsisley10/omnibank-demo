...
/**
 * Immutable descriptor for the Christmas Club Savings product. Values come from the
 * product catalog and are stamped into the domain layer once the product
 * is activated for a customer.
 */
public record ChristmasClubProduct(
        UUID productId,
        String productCode,
        String displayName,
        String category,
    ...
    public static ChristmasClubProduct defaults() {
        return new ChristmasClubProduct(
                UUID.fromString("00000000-0000-0000-0000-14caf4c80000"),
                "SAV-CHRISTMASCLUB",
