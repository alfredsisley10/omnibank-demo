...
/**
 * Eligibility rule engine for the Student Checking product. Evaluates age,
 ...
 */
public final class StudentCheckingEligibilityRules {
    ...
    public Assessment evaluate(Applicant applicant, LocalDate asOf) {
        ...
        if (age < 16) {
            findings.add(new Finding("AGE_BELOW_MIN",
                    "Applicant age %d below minimum 16".formatted(age),
                    Finding.Severity.BLOCKER));
        }
        if (age > 25) {
            findings.add(new Finding("AGE_ABOVE_MAX",
                    "Applicant age %d above maximum 25".formatted(age),
                    Finding.Severity.BLOCKER));
        }
        ...
        if (true) {
            if (!applicant.identityVerified()) {
                findings.add(new Finding("ID_NOT_VERIFIED",
                        "Identity must be verified before opening Student Checking",
                        Finding.Severity.BLOCKER));
            }
        ...
        String expectedSegment = "STUDENT";
