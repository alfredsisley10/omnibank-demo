...
/**
 * Eligibility rule engine for the First Account Savings product. Evaluates age,
 ...
 */
public final class FirstAccountSavingsEligibilityRules {
    ...
    public Assessment evaluate(Applicant applicant, LocalDate asOf) {
        ...
        if (age > 12) {
            findings.add(new Finding("AGE_ABOVE_MAX",
                    "Applicant age %d above maximum 12".formatted(age),
                    Finding.Severity.BLOCKER));
        }
        ...
        if (true) {
            if (!applicant.identityVerified()) {
                findings.add(new Finding("ID_NOT_VERIFIED",
                        "Identity must be verified before opening First Account Savings",
                        Finding.Severity.BLOCKER));
            }
