...
class DigitalOnlyCheckingNMProfileTest {

    @Test
    void profile_reports_expected_state_code() {
        var p = new DigitalOnlyCheckingNMProfile();
        assertThat(p.stateCode()).isEqualTo("NM");
        assertThat(p.stateName()).isEqualTo("New Mexico");
    }
    ...
    void rate_table_returns_positive_apy_for_low_balance() {
        var rt = new DigitalOnlyCheckingNMRateTable();
    ...
    void disclosures_contain_state_name() {
        var d = new DigitalOnlyCheckingNMDisclosures();
        assertThat(d.renderFullSupplement()).contains("New Mexico");
    }

    @Test
    void regulatory_filings_produced_for_quarter() {
        var f = new DigitalOnlyCheckingNMRegulatoryFilings();
    ...
    void channel_integration_lists_web_and_mobile() {
        var b = new DigitalOnlyCheckingNMBranchIntegration();
