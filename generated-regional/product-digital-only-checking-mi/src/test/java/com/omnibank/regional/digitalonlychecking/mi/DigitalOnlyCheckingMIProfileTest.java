...
class DigitalOnlyCheckingMIProfileTest {

    @Test
    void profile_reports_expected_state_code() {
        var p = new DigitalOnlyCheckingMIProfile();
        assertThat(p.stateCode()).isEqualTo("MI");
        assertThat(p.stateName()).isEqualTo("Michigan");
    }
    ...
    void rate_table_returns_positive_apy_for_low_balance() {
        var rt = new DigitalOnlyCheckingMIRateTable();
    ...
    void disclosures_contain_state_name() {
        var d = new DigitalOnlyCheckingMIDisclosures();
        assertThat(d.renderFullSupplement()).contains("Michigan");
    }

    @Test
    void regulatory_filings_produced_for_quarter() {
        var f = new DigitalOnlyCheckingMIRegulatoryFilings();
    ...
    void channel_integration_lists_web_and_mobile() {
        var b = new DigitalOnlyCheckingMIBranchIntegration();
