...
class DigitalOnlyCheckingIAProfileTest {

    @Test
    void profile_reports_expected_state_code() {
        var p = new DigitalOnlyCheckingIAProfile();
        assertThat(p.stateCode()).isEqualTo("IA");
        assertThat(p.stateName()).isEqualTo("Iowa");
    }
    ...
    void rate_table_returns_positive_apy_for_low_balance() {
        var rt = new DigitalOnlyCheckingIARateTable();
    ...
    void disclosures_contain_state_name() {
        var d = new DigitalOnlyCheckingIADisclosures();
        assertThat(d.renderFullSupplement()).contains("Iowa");
    }

    @Test
    void regulatory_filings_produced_for_quarter() {
        var f = new DigitalOnlyCheckingIARegulatoryFilings();
    ...
    void channel_integration_lists_web_and_mobile() {
        var b = new DigitalOnlyCheckingIABranchIntegration();
