...
class DigitalOnlyCheckingHIProfileTest {

    @Test
    void profile_reports_expected_state_code() {
        var p = new DigitalOnlyCheckingHIProfile();
        assertThat(p.stateCode()).isEqualTo("HI");
        assertThat(p.stateName()).isEqualTo("Hawaii");
    }
    ...
    void rate_table_returns_positive_apy_for_low_balance() {
        var rt = new DigitalOnlyCheckingHIRateTable();
    ...
    void disclosures_contain_state_name() {
        var d = new DigitalOnlyCheckingHIDisclosures();
        assertThat(d.renderFullSupplement()).contains("Hawaii");
    }

    @Test
    void regulatory_filings_produced_for_quarter() {
        var f = new DigitalOnlyCheckingHIRegulatoryFilings();
    ...
    void channel_integration_lists_web_and_mobile() {
        var b = new DigitalOnlyCheckingHIBranchIntegration();
