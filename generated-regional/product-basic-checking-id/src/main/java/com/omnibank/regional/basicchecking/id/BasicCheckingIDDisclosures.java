package com.omnibank.regional.basicchecking.id;
...
/**
 * State-specific disclosure overlays for BasicChecking in Idaho.
 * Layered on top of the national disclosure, these capture
 * items that only apply under ID law (UDAP, UIPA, unclaimed
 * property reporting, language access, electronic signature).
 */
public final class BasicCheckingIDDisclosures {

    private final BasicCheckingIDProfile profile;

    public BasicCheckingIDDisclosures() {
        this.profile = new BasicCheckingIDProfile();
    }

    public BasicCheckingIDDisclosures(BasicCheckingIDProfile profile) {
    ...
    public String stateHeader() {
        return "Idaho Residents Disclosure — Effective "
    ...
    public String unclaimedPropertyClause() {
        return "Idaho unclaimed property law requires abandoned funds to be "
    ...
    public String printedCopyRequiredClause() {
        return "A printed copy of this disclosure will be provided at account opening "
                + "as required by Idaho law, in addition to any electronic copy delivered.";
    }

    public String noSalesTaxClause() {
        return "Idaho does not impose sales tax on bank service fees; "
    ...
    public String renderFullSupplement() {
        ...
        sb.append("BasicChecking — Idaho State Disclosure Supplement\n");
        sb.append("=".repeat(70)).append("\n\n");
