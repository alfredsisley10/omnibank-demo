...
/**
 * Driver for the nightly batch belonging to the retired Legacy Velocity Monitor.
 *
...
public final class LegacyVelocityMonitorBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyVelocityMonitorBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyVelocityMonitorDao dao;
    private final LegacyVelocityMonitorEventListener listener;

    public LegacyVelocityMonitorBatchDriver(Clock clock, LegacyVelocityMonitorDao dao, LegacyVelocityMonitorEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyVelocityMonitor batch retired 2019 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code VelocityCheckEngine}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2019-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyVelocityMonitor would start at {} but is retired (MIG-4008)",
                start);
        return List.of();
    }
}
