...
/**
 * Driver for the nightly batch belonging to the retired Legacy Settlement Batcher.
 *
...
public final class LegacySettlementBatcherBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacySettlementBatcherBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacySettlementBatcherDao dao;
    private final LegacySettlementBatcherEventListener listener;

    public LegacySettlementBatcherBatchDriver(Clock clock, LegacySettlementBatcherDao dao, LegacySettlementBatcherEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacySettlementBatcher batch retired 2020 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code CardSettlementProcessor}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2020-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacySettlementBatcher would start at {} but is retired (MIG-7015)",
                start);
        return List.of();
    }
}
