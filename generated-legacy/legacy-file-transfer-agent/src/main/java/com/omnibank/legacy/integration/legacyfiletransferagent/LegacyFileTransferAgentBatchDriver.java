...
/**
 * Driver for the nightly batch belonging to the retired Legacy File Transfer Agent.
 *
...
public final class LegacyFileTransferAgentBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyFileTransferAgentBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyFileTransferAgentDao dao;
    private final LegacyFileTransferAgentEventListener listener;

    public LegacyFileTransferAgentBatchDriver(Clock clock, LegacyFileTransferAgentDao dao, LegacyFileTransferAgentEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyFileTransferAgent batch retired 2018 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code SecureFileTransferService}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2018-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyFileTransferAgent would start at {} but is retired (MIG-9701)",
                start);
        return List.of();
    }
}
