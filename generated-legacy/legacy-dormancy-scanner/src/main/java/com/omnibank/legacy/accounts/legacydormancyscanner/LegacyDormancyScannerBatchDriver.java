...
/**
 * Driver for the nightly batch belonging to the retired Legacy Dormancy Scanner.
 *
...
public final class LegacyDormancyScannerBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyDormancyScannerBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyDormancyScannerDao dao;
    private final LegacyDormancyScannerEventListener listener;

    public LegacyDormancyScannerBatchDriver(Clock clock, LegacyDormancyScannerDao dao, LegacyDormancyScannerEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyDormancyScanner batch retired 2020 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code AccountStateMachine}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2020-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyDormancyScanner would start at {} but is retired (MIG-2031)",
                start);
        return List.of();
    }
}
