...
/**
 * Driver for the nightly batch belonging to the retired Legacy ACH Processor.
 *
...
public final class LegacyAchProcessorBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyAchProcessorBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyAchProcessorDao dao;
    private final LegacyAchProcessorEventListener listener;

    public LegacyAchProcessorBatchDriver(Clock clock, LegacyAchProcessorDao dao, LegacyAchProcessorEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyAchProcessor batch retired 2019 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code AchPaymentProcessor}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2019-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyAchProcessor would start at {} but is retired (MIG-1042)",
                start);
        return List.of();
    }
}
