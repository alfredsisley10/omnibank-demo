...
/**
 * Driver for the nightly batch belonging to the retired Legacy Email Sender.
 *
...
public final class LegacyEmailSenderBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyEmailSenderBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyEmailSenderDao dao;
    private final LegacyEmailSenderEventListener listener;

    public LegacyEmailSenderBatchDriver(Clock clock, LegacyEmailSenderDao dao, LegacyEmailSenderEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyEmailSender batch retired 2019 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code EmailDeliveryService}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2019-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyEmailSender would start at {} but is retired (MIG-9008)",
                start);
        return List.of();
    }
}
