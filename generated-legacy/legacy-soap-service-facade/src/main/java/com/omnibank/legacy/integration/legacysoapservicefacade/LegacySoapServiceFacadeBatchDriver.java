...
/**
 * Driver for the nightly batch belonging to the retired Legacy SOAP Service Facade.
 *
...
public final class LegacySoapServiceFacadeBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacySoapServiceFacadeBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacySoapServiceFacadeDao dao;
    private final LegacySoapServiceFacadeEventListener listener;

    public LegacySoapServiceFacadeBatchDriver(Clock clock, LegacySoapServiceFacadeDao dao, LegacySoapServiceFacadeEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacySoapServiceFacade batch retired 2022 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code RestServiceFacade}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2022-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacySoapServiceFacade would start at {} but is retired (MIG-9729)",
                start);
        return List.of();
    }
}
