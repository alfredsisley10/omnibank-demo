...
/**
 * Driver for the nightly batch belonging to the retired Legacy EOD Close Runner.
 *
...
public final class LegacyEodCloseRunnerBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyEodCloseRunnerBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyEodCloseRunnerDao dao;
    private final LegacyEodCloseRunnerEventListener listener;

    public LegacyEodCloseRunnerBatchDriver(Clock clock, LegacyEodCloseRunnerDao dao, LegacyEodCloseRunnerEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyEodCloseRunner batch retired 2019 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code EndOfDayCloseManager}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2019-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyEodCloseRunner would start at {} but is retired (MIG-9608)",
                start);
        return List.of();
    }
}
