...
/**
 * Driver for the nightly batch belonging to the retired Legacy Preferences Portal.
 *
...
public final class LegacyPreferencesPortalBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyPreferencesPortalBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyPreferencesPortalDao dao;
    private final LegacyPreferencesPortalEventListener listener;

    public LegacyPreferencesPortalBatchDriver(Clock clock, LegacyPreferencesPortalDao dao, LegacyPreferencesPortalEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyPreferencesPortal batch retired 2021 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code StatementPreferencesManager}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2021-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyPreferencesPortal would start at {} but is retired (MIG-6022)",
                start);
        return List.of();
    }
}
