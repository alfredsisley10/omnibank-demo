...
/**
 * Driver for the nightly batch belonging to the retired Legacy Card Auth Gateway.
 *
...
public final class LegacyCardAuthGatewayBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyCardAuthGatewayBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyCardAuthGatewayDao dao;
    private final LegacyCardAuthGatewayEventListener listener;

    public LegacyCardAuthGatewayBatchDriver(Clock clock, LegacyCardAuthGatewayDao dao, LegacyCardAuthGatewayEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyCardAuthGateway batch retired 2018 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code CardAuthorizationEngine}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2018-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyCardAuthGateway would start at {} but is retired (MIG-7001)",
                start);
        return List.of();
    }
}
