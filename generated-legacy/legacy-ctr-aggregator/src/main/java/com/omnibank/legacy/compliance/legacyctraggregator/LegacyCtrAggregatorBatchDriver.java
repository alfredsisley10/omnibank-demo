...
/**
 * Driver for the nightly batch belonging to the retired Legacy CTR Aggregator.
 *
...
public final class LegacyCtrAggregatorBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyCtrAggregatorBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyCtrAggregatorDao dao;
    private final LegacyCtrAggregatorEventListener listener;

    public LegacyCtrAggregatorBatchDriver(Clock clock, LegacyCtrAggregatorDao dao, LegacyCtrAggregatorEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyCtrAggregator batch retired 2021 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code CurrencyTransactionReporter}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2021-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyCtrAggregator would start at {} but is retired (MIG-3022)",
                start);
        return List.of();
    }
}
