...
/**
 * Driver for the nightly batch belonging to the retired Legacy E-Statement Sender.
 *
...
public final class LegacyEStatementSenderBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyEStatementSenderBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyEStatementSenderDao dao;
    private final LegacyEStatementSenderEventListener listener;

    public LegacyEStatementSenderBatchDriver(Clock clock, LegacyEStatementSenderDao dao, LegacyEStatementSenderEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyEStatementSender batch retired 2020 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code StatementDeliveryService}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2020-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyEStatementSender would start at {} but is retired (MIG-6008)",
                start);
        return List.of();
    }
}
