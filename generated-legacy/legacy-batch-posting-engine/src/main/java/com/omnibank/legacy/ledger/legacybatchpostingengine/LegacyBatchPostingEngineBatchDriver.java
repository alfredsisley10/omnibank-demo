...
/**
 * Driver for the nightly batch belonging to the retired Legacy Batch Posting Engine.
 *
...
public final class LegacyBatchPostingEngineBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyBatchPostingEngineBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyBatchPostingEngineDao dao;
    private final LegacyBatchPostingEngineEventListener listener;

    public LegacyBatchPostingEngineBatchDriver(Clock clock, LegacyBatchPostingEngineDao dao, LegacyBatchPostingEngineEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyBatchPostingEngine batch retired 2018 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code PostingService}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2018-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyBatchPostingEngine would start at {} but is retired (MIG-1102)",
                start);
        return List.of();
    }
}
