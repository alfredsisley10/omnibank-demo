...
/**
 * Driver for the nightly batch belonging to the retired Legacy Chargeback Processor.
 *
...
public final class LegacyChargebackProcessorBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyChargebackProcessorBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyChargebackProcessorDao dao;
    private final LegacyChargebackProcessorEventListener listener;

    public LegacyChargebackProcessorBatchDriver(Clock clock, LegacyChargebackProcessorDao dao, LegacyChargebackProcessorEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyChargebackProcessor batch retired 2020 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code CardDisputeManager}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2020-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyChargebackProcessor would start at {} but is retired (MIG-4015)",
                start);
        return List.of();
    }
}
