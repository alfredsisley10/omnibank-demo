...
/**
 * Driver for the nightly batch belonging to the retired Legacy Fee Assessor.
 *
...
public final class LegacyFeeAssessorBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyFeeAssessorBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyFeeAssessorDao dao;
    private final LegacyFeeAssessorEventListener listener;

    public LegacyFeeAssessorBatchDriver(Clock clock, LegacyFeeAssessorDao dao, LegacyFeeAssessorEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyFeeAssessor batch retired 2021 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code AccountFeeEngine}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2021-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyFeeAssessor would start at {} but is retired (MIG-2025)",
                start);
        return List.of();
    }
}
