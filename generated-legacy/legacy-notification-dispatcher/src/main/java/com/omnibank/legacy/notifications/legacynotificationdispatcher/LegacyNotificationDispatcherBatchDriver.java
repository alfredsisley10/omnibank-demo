...
/**
 * Driver for the nightly batch belonging to the retired Legacy Notification Dispatcher.
 *
...
public final class LegacyNotificationDispatcherBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyNotificationDispatcherBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyNotificationDispatcherDao dao;
    private final LegacyNotificationDispatcherEventListener listener;

    public LegacyNotificationDispatcherBatchDriver(Clock clock, LegacyNotificationDispatcherDao dao, LegacyNotificationDispatcherEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyNotificationDispatcher batch retired 2018 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code NotificationDispatcher}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2018-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyNotificationDispatcher would start at {} but is retired (MIG-9001)",
                start);
        return List.of();
    }
}
