...
/**
 * Driver for the nightly batch belonging to the retired Legacy Statement Renderer.
 *
...
public final class LegacyStatementRendererBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyStatementRendererBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyStatementRendererDao dao;
    private final LegacyStatementRendererEventListener listener;

    public LegacyStatementRendererBatchDriver(Clock clock, LegacyStatementRendererDao dao, LegacyStatementRendererEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyStatementRenderer batch retired 2018 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code StatementGenerator}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2018-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyStatementRenderer would start at {} but is retired (MIG-6001)",
                start);
        return List.of();
    }
}
