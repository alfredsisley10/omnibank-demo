...
/**
 * Driver for the nightly batch belonging to the retired Legacy Check Clearing Service.
 *
...
public final class LegacyCheckClearingServiceBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyCheckClearingServiceBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyCheckClearingServiceDao dao;
    private final LegacyCheckClearingServiceEventListener listener;

    public LegacyCheckClearingServiceBatchDriver(Clock clock, LegacyCheckClearingServiceDao dao, LegacyCheckClearingServiceEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyCheckClearingService batch retired 2020 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code CheckClearingProcessor}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2020-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyCheckClearingService would start at {} but is retired (MIG-1078)",
                start);
        return List.of();
    }
}
