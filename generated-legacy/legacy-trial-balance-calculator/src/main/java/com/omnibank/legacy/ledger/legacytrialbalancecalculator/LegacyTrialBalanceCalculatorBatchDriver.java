...
/**
 * Driver for the nightly batch belonging to the retired Legacy Trial Balance Calculator.
 *
...
public final class LegacyTrialBalanceCalculatorBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyTrialBalanceCalculatorBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyTrialBalanceCalculatorDao dao;
    private final LegacyTrialBalanceCalculatorEventListener listener;

    public LegacyTrialBalanceCalculatorBatchDriver(Clock clock, LegacyTrialBalanceCalculatorDao dao, LegacyTrialBalanceCalculatorEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyTrialBalanceCalculator batch retired 2019 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code TrialBalanceEngine}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2019-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyTrialBalanceCalculator would start at {} but is retired (MIG-1110)",
                start);
        return List.of();
    }
}
