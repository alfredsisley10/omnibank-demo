...
/**
 * Driver for the nightly batch belonging to the retired Legacy Archive Service.
 *
...
public final class LegacyArchiveServiceBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyArchiveServiceBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyArchiveServiceDao dao;
    private final LegacyArchiveServiceEventListener listener;

    public LegacyArchiveServiceBatchDriver(Clock clock, LegacyArchiveServiceDao dao, LegacyArchiveServiceEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyArchiveService batch retired 2019 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code StatementArchiveService}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2019-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyArchiveService would start at {} but is retired (MIG-6015)",
                start);
        return List.of();
    }
}
