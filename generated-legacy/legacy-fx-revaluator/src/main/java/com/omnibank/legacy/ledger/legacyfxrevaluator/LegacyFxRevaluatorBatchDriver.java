...
/**
 * Driver for the nightly batch belonging to the retired Legacy FX Revaluator.
 *
...
public final class LegacyFxRevaluatorBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyFxRevaluatorBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyFxRevaluatorDao dao;
    private final LegacyFxRevaluatorEventListener listener;

    public LegacyFxRevaluatorBatchDriver(Clock clock, LegacyFxRevaluatorDao dao, LegacyFxRevaluatorEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyFxRevaluator batch retired 2022 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code CurrencyRevaluationEngine}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2022-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyFxRevaluator would start at {} but is retired (MIG-1124)",
                start);
        return List.of();
    }
}
