...
/**
 * Driver for the nightly batch belonging to the retired Legacy Stress Test Runner.
 *
...
public final class LegacyStressTestRunnerBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyStressTestRunnerBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyStressTestRunnerDao dao;
    private final LegacyStressTestRunnerEventListener listener;

    public LegacyStressTestRunnerBatchDriver(Clock clock, LegacyStressTestRunnerDao dao, LegacyStressTestRunnerEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyStressTestRunner batch retired 2020 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code StressTestHarness}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2020-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyStressTestRunner would start at {} but is retired (MIG-8008)",
                start);
        return List.of();
    }
}
