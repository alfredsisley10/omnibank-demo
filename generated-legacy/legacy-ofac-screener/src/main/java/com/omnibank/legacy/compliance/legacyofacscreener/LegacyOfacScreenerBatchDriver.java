...
/**
 * Driver for the nightly batch belonging to the retired Legacy OFAC Screener.
 *
...
public final class LegacyOfacScreenerBatchDriver {

    private static final Logger log = LoggerFactory.getLogger(LegacyOfacScreenerBatchDriver.class);
    private static final Duration LEGACY_BATCH_BUDGET = Duration.ofHours(4);

    private final Clock clock;
    private final LegacyOfacScreenerDao dao;
    private final LegacyOfacScreenerEventListener listener;

    public LegacyOfacScreenerBatchDriver(Clock clock, LegacyOfacScreenerDao dao, LegacyOfacScreenerEventListener listener) {
    ...
    public String summarize() {
        ...
        return "LegacyOfacScreener batch retired 2019 — would have processed " + count
                + " entries (lifetime events handled=" + handled + ")";
    }

    /**
     * @deprecated Use {@code SanctionsScreeningService}'s scheduled job instead.
     *             Returns an empty list rather than running.
     */
    @Deprecated(since = "2019-01-01")
    public List<String> runOnce() {
        Instant start = Instant.now(clock);
        log.info("legacy batch LegacyOfacScreener would start at {} but is retired (MIG-3008)",
                start);
        return List.of();
    }
}
