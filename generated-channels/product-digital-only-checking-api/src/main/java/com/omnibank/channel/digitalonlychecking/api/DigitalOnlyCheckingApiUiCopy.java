package com.omnibank.channel.digitalonlychecking.api;

import java.util.Locale;
import java.util.Map;

/**
 * Channel-tailored UI / audio copy for the DigitalOnlyChecking account flow over
 * Third-Party API. Copy length and vocabulary differ: web has room for
 * long-form copy, ATM screens must fit a narrow line, IVR text is
 * read aloud and must avoid visual-only cues.
 */
public final class DigitalOnlyCheckingApiUiCopy {

    private final boolean supportsSpanish;

    public DigitalOnlyCheckingApiUiCopy() {
        this.supportsSpanish = false;
    }

    public String welcomeHeadline(Locale locale) {
        if (isSpanish(locale)) {
            return "Bienvenido a su cuenta digitalonlychecking";
        }
        return "Welcome to your digitalonlychecking account";
    }
